
import 'package:flutter/material.dart';

class Constants {
  // Colors
  static const Color primaryColor = Color.fromARGB(255, 244, 168, 55);
  static const Color secondaryColor = Color(0xFF2196F3);
  static const Color backgroundColor = Color(0xFFF1F1F1);
  static const Color buttonColor = Color(0xFF1E88E5);
 
  
  // Spacing
  static const double horizontalPadding = 40.0;
  static const double verticalPadding = 24.0;
  static const double fieldHeight = 50.0;
  static const double buttonHeight = 45.0;
  static const double fieldSpacing = 16.0;

  // Font sizes
  static const double titleFontSize = 24.0;
  static const double subtitleFontSize = 18.0;
  static const double mobiletitleFontSize = 16.0;
  static  const double mobilesubtitleFontSize = 8.0;
}

